...
fn generate_elaborating() {
    let mut buffer = generated_file("elaborating_generated.rs");
    writeln!(buffer, r#"// Do not edit! See build.rs

#[rustfmt::skip]
fn run_test(folder: &str, file: &str) {{
    let path = std::path::Path::new("fixtures/elaborating").join(folder);
    let (engine, _) = tests_integration::load_compiler(&path);
    let Some(id) = engine.module_file(file) else {{ return }};
    let report = tests_integration::generated::basic::report_elaborated(&engine, id);
    let mut settings = insta::Settings::clone_current();
    settings.set_snapshot_path(std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures/elaborating").join(folder));
    settings.set_prepend_module_to_snapshot(false);
    settings.bind(|| insta::assert_snapshot!(file, report));
}}"#).unwrap();
